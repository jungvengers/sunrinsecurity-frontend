...
const initialState = {
  ...
  uploadNotice: reducerUtils.initial(),
};
...
const form = handleActions(
  {
    ...
    [actions.REMOVE_IMAGE_FILE]: (state, action) => ({
      ...
      imageFiles: state.imageFiles.filter(
        (imageFile) => imageFile.name !== action.payload.name
      ),
    }),
    ...
    // 글 업로드
    [actions.UPLOAD_NOTICE_LOADING]: (state, action) => ({
      ...state,
      uploadNotice: reducerUtils.loading(),
    }),
    [actions.UPLOAD_NOTICE_SUCCESS]: (state, action) => ({
      ...state,
      uploadNotice: reducerUtils.success(action.payload),
    }),
    [actions.UPLOAD_NOTICE_ERROR]: (state, action) => ({
      ...state,
      uploadNotice: reducerUtils.error(action.payload),
    }),
  },
  initialState
);
